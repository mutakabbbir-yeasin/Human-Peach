<?php
  ...
  if(!empty($_POST))
  {
    ...
    $slug=uniqid('LAWYER');
     ...
     $image=$_FILES['photo'];
     $imageName='lawyer_'.time().'_'.rand(100000,1000000).'.'.pathinfo($image['name'],PATHINFO_EXTENSION);
    
     $insert="insert into user(name,email,mobile,address,study,working_place,nid,joining_date,password,photo,slug,role_id) 
     values ('$name','$email','$mobile','$address','$study','$working_place','$nid','$joining_date','$password','$imageName','$slug','2')";
     ...
     if($password==$repass){
      ...
      if($Q){
       ...
       header('Location: all-lawyer.php');
      }
     }else{
