<?php
  ...
  if(!empty($_POST))
  {
    ...
    $slug=uniqid('COUNSELLOR');
     ...
     $image=$_FILES['photo'];
     $imageName='counsellor_'.time().'_'.rand(100000,1000000).'.'.pathinfo($image['name'],PATHINFO_EXTENSION);
     ...
     if($password==$repass){
      ...
      if($Q){
       ...
       header('Location: all-counsellor.php');
      }
     }else{
       $_SESSION['success_alert']='8';
       header('Location: add-counsellor.php');
     }
